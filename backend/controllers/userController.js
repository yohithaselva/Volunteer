...
export const logoutUser = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Logout successful', record: result.rows[0] });
  } catch (err) {
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO Users (username, password, role, email, phone, year, department) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      [username, hashedPassword, role, email, phone, year, department]
    );
...
export const getVolunteerHours = async (req, res) => {
  ...
  try {
    let query = `
      SELECT user_id, 
             SUM(EXTRACT(EPOCH FROM (checkout_time - checkin_time)) / 3600   total_hours
