...
export const createAssignment = async (req, res) => {
  const { task_id, user_id, priority_level, estimated_completion_time } = req.body;
  try {
...
export const updateAssignmentStatus = async (req, res) => {
  ...
  try {
    ...
    if (status === 'Completed') {
      ...
      await pool.query(
        'INSERT INTO Notifications (user_id, message) VALUES ($1, $2)',
        [assignment.user_id, `Task ID ${assignment.task_id} has been completed.`]
      );
    }
...
export const deleteAssignment = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Assignment deleted', deletedAssignment: result.rows[0] });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
