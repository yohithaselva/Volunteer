...
function VolunteerLogViewer() {
  ...
  useEffect(() => {
    const fetchCheckIns = async () => {
      try {
        const data = await getAllcheckins(); // Use the API function
        setLogs(data);
      } catch (error) {
  ...
  // Fetch total hours from the backend
  useEffect(() => {
    const fetchTotalHours = async () => {
      try {
        const data = await getAllhours(); // Use the API function
        setTotalHours(data.total_hours || 0); // Fixed: access total_hours directly
      } catch (error) {
  ...
  const filteredLogs = logs.filter((log) => {
    return (
      (filter.date ? new Date(log.checkin_time).toLocaleDateString().includes(filter.date) : true) && // Fixed: format date consistently for filtering
      (filter.volunteer
        ? log.user_id.toString().toLowerCase().includes(filter.volunteer.toLowerCase())
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-800 to-blue-900 p-4 md:p-8 text-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 mb-6 overflow-x-auto"> {/* Fixed: added overflow-x-auto for mobile responsiveness */}
          {/* Logs Table */}
              ...
              {filteredLogs.length > 0 ? (
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-white/10 transition-all">
                    <td className="border px-4 py-2">{new Date(log.checkin_time).toLocaleDateString()}</td>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6">
          ...
          <p className="text-lg">Total Hours Logged: {totalHours.toFixed(2)}</p> {/* Fixed: format with 2 decimal places */}
        </div>
      </div>
    </div>
  );
